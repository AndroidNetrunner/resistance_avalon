...
async def next_vote(embed):
	game_status['leader'] = game_room['members'][(game_room['members'].index(game_status['leader']) + 1) % len(game_room['members'])]
...
async def add_teammate(payload, player):
	...
	if payload.user_id == game_status['leader'].id:
		...
		await game_status['leader'].send(f"{player.name}님이 원정대에 추가되었습니다.")
		if len(current_round['team']) == quest_sheet[len(game_room['members'])][game_status['round'] - 1]:
...
async def start_voting(team):
	embed = discord.Embed(
		title=f"원정대장이 {game_status['round']}라운드 {current_round['decision']}번째 팀을 결정했습니다.")
	str_team = ""

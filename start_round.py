...
async def decide_team(num):
	...
	embed = discord.Embed(title="원정대장님, 원정대를 결성해주셔야 합니다.",
	                      description=f"이번 라운드에 데려갈 인원은 {quest_sheet[len(game_room['members'])][current_round['round'] - 1]}명입니다.")
...
async def start_round(num):
	...
	embed.add_field(name=f"현재 원정대장은 {current_round['leader'].name}입니다.",
	                value=f"이번 라운드에 데려갈 인원은 {quest_sheet[len(game_room['members'])][num]}명입니다.")
	await game_room['main_channel'].send(embed=embed)
...
async def start_voting(team):
	embed = discord.Embed(
		title=f"원정대장이 {current_round['round']}라운드 {current_round['decision'] + 1}번째 팀을 결정했습니다.")
	str_team = ""
